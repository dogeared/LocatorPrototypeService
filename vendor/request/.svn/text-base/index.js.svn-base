...
function request (options, callback) {
  ...
  if (options.proxy) {
    ...
    options.client = options.client ? options.client : http.createClient(options.proxy.port, options.proxy.hostname, secure, options.credentials);
  } else {
    var secure = (options.uri.protocol == 'https:') ? true : false
    options.client = options.client ? options.client : http.createClient(options.uri.port, options.uri.hostname, secure, options.credentials);
  }
